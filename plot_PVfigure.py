...
if __name__ == "__main__":
	...
	for i,sigma in enumerate(sigmas):
	    PV_avg[i] = np.mean(results[i]['rPa'][450000:])
	    PV_std[i] = np.std(results[i]['rPa'][450000:])
	    wPX_avg[i] = np.mean(results[i]['wPX1'][450000:])
	    wPX_std[i] = np.std(results[i]['wPX1'][450000:])
	...
	sigma2=0.4
	sim_time= 500000
	PV_avg1 = np.mean(results1['rPa'][450000:])
	PV_avg2 = np.mean(results2['rPa'][450000:])
	PV_std1 = np.std(results1['rPa'][450000:])
	PV_std2 = np.std(results2['rPa'][450000:])
